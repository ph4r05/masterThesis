...
% attacker def
    \emph{Whitebox context} (also abbreviated as WBC) is itself defined by the attacker model. WBC attacker has full control over execution of particular 
    ...
    In contrast to \emph{blackbox context} (also abreviated as BBC) that has only access to output of the cryptographic algorithm. In BBC the cryptographic algorithm is
    ...
    (representing the right owner) is executed in potentionaly hostile environment, where user can have motivation to extract protected content without
