...
\chapter{Whitebox context attack resistant AES implementation using dual AES}
    ...
    \begin{align}\label{eq:twofish_sbox}
...
% 	\hline
    ...
    \begin{itemize}
     ...
     \item[$k^{r-1}$] is whole round key for roudn $r-1$
	...
	hash_{N_{bc}, N_{sha}}(k^{r-1} \; || \; key \; || \; "\text{magicConstant}", salt)_i            & \quad \text{otherwise}
    \end{array} \right.
    \end{equation}
    ...
    Thus pre-computed $s_0$ s-box would take $2^8 \cdot 2^{16} = 2^{24} > 10^7$ bytes. 
	...
	sboxgen(j,l,x) = \left\{ 
	\begin{array}{l l} 
	    q^{\prime}_{j,0}\left[ x \right] \oplus k_{j}		 & \quad \text{if $l=0$}\\
	    sboxgen\left(j, l-1,x\right) \oplus k_{j \cdot (l+1)}          & \quad \text{otherwise}
	\end{array} \right.
    \end{equation}
    ...
    This gives us good security margin for BGE attack. Also as long as are SHA-256 and bcrypt uncracked, key extraction should be unfeasible, since it is not possible
    to invert hash function easily. 
    
    Suggested modification of whitebox cipher don't increase table sizes, because modifications made are only in S-box definitions and key schedule - both 
