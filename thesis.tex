...
% regex test
% 
% a~b $1~$2 $1~$2 $1~$2 u~r e~cd f~h r~s e~f f~s w~r g~v
...
\chapter{Whitebox context attack resistant AES implementation using dual AES}
    ...
    $\left(2^{128} \cdot 128\right) > 10^{38}$ bytes. This 
    ...
    \begin{align}
	s_0\left(x\right) &= q_1\left[q_0\left[q_0\left[x\right] \oplus k_0 \right] \oplus k_1 \right]\\
	s_1\left(x\right) &= q_0\left[q_0\left[q_1\left[x\right] \oplus k_2 \right] \oplus k_3 \right]\\
	s_2\left(x\right) &= q_1\left[q_1\left[q_0\left[x\right] \oplus k_4 \right] \oplus k_5 \right]\\
	s_3\left(x\right) &= q_0\left[q_1\left[q_1\left[x\right] \oplus k_6 \right] \oplus k_7 \right]
    \end{align}
    Where $q_0, q_1$ are fixed 8-bit permutations, $k_i,\; i \in [0,7]$ are key bytes, $s_j,\; j \in [0,4]$ are resulting S-boxes.
    ...
    S-box definitions are needed in proposition 3 in BGE attack
...
\begin{thebibliography}{9}
	\bibitem{twofish} \uppercase{SCHNEIRER, B. KELSEY, J., Whiting, D., Wagner, D., Hall, C., Ferguson, N.} , Twofish: A 128-Bit Block Cipher \textless\url{http://www.schneier.com/paper-twofish-paper.pdf}, cit.~14.05.2012.
